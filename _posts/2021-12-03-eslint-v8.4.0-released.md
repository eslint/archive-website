...
The `context` argument has a new property `cwd` (string, absolute path). It represents the current working directory used for linting. This value comes from the `cwd` constructor option of the [ESLint](/docs/developer-guide/nodejs-api.md#-new-eslintoptions) class, and formatters can use it to output relative paths more reliably than by using `process.cwd()`.
